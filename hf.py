...
def gauss_newton_product(cost, params, vars, obj):
    ...
    Gv = tt.grad(tt.sum(HJv * obj), params,
                consider_constant=[HJv, Jv], disconnected_inputs='ignore')
    Gv = map(tt.as_tensor_variable, Gv)  # for CudaNdarray
    return Gv
...
class hf_optimizer:
    ...
    def __init__(self, params, inputs, obj, costs, h=None, ha=None):
        """Constructs and compiles the necessary Theano functions.
            ...
            inputs : list of Theano variables
                    Symbolic variables that are inputs to your graph
                    (they should also include your model 'output').
                    Your training examples must fit these.

            s : Theano variable
                Symbolic variable with respect to which the Hessian of the
                objective is positive-definite, implicitly defining the
                Gauss-Newton matrix. Typically, it is the activation of the
                output layer.
            ...
            h: Theano variable or None
                    Structural damping is applied to this variable
                    (typically the hidden units of an RNN).

            ha: Theano variable or None
                Symbolic variable that implicitly defines the Gauss-Newton
                matrix for the structural damping term (typically the
                activation of the hidden layer). If None, it will be set to
                `h`.
                """

        self.p = params
        self.shapes = [i.get_value().shape for i in params]
        self.sizes = [np.prod(s) for s in self.shapes]
        self.positions = np.cumsum([0] + self.sizes)[:-1]

        g = tt.grad(costs[0], params)
        g = [tt.as_tensor_variable(gx) for gx in g]  # for CudaNdarray
        self.f_gc = theano.function(inputs, g + costs,
                                    on_unused_input='ignore')
                                    # during gradient computation
        self.f_cost = theano.function(inputs, costs,
                                      on_unused_input='ignore')
                                      # for quick cost evaluation

        symbolic_types = tt.scalar, tt.vector, tt.matrix, tt.tensor3, tt.tensor4

        v = [symbolic_types[len(i)]() for i in self.shapes]
        Gv = gauss_newton_product(costs[0], params, v, obj)

        coefficient = tt.scalar()  # this is lambda*mu
        ...
        if h is not None:  # structural damping with cross-entropy
            ...
            structural_damping = coefficient * (
                -h_constant * tt.log(h + 1e-10) - (1 - h_constant) * tt.log(
                    (1 - h) + 1e-10)).sum() / h.shape[0]

            if ha is None:
                ha = h

            Gv_damping = gauss_newton_product(structural_damping, params, v, ha)
        ...
        self.function_Gv = theano.function(inputs + v + [coefficient], Gv,
                                           givens=givens,
                                           on_unused_input='ignore')
    ...
    def quick_cost(self, delta=0):
        ...
        cost = np.mean(
            [self.f_cost(*i)[0] for i in self.cg_dataset.iterate(update=False)])
    ...
    def cg(self, b):
        if self.preconditioner:
            ...
            for inputs in self.cg_dataset.iterate(update=False):
                M += list_to_flat(self.f_gc(*inputs)[:len(self.p)]) ** 2
            # / self.cg_dataset.number_batches**2
        ...
        x = self.cg_last_x if hasattr(self, 'cg_last_x') \
            else np.zeros_like(b)  # sharing information between CG runs
        r = b - self.batch_Gv(x)
        d = M * r
        ...
        for i in range(1, 1 + self.max_cg_iterations):
            ...
            progress = ' [CG iter %i, phi=%+.5f, cost=%.5f]' % (
                i, phi_i, backtracking[-1][0])
            sys.stdout.write('\b' * backspaces + progress)
            sys.stdout.flush()
            backspaces = len(progress)

            k = max(10, i / 10)
            if i > k and phi_i < 0 and (
                        phi_i - phi[-k - 1]) / phi_i < k * 0.0005:
                break
    ...
    def flat_to_list(self, vector):
        return [vector[position:position + size].reshape(shape) for
                shape, size, position in
    ...
    def train(self, gradient_dataset, cg_dataset, initial_lambda=0.1, mu=0.03,
              global_backtracking=False, preconditioner=False,
              max_cg_iterations=250, num_updates=100, validation=None,
              validation_frequency=1, patience=np.inf, save_progress=None):
    ...
    save_progress: string or None
        ...
        try:
            for u in range(first_iteration, 1 + num_updates):
                ...
                gradient = np.zeros(sum(self.sizes),
                                       dtype=theano.config.floatX)
                costs = []
                for inputs in gradient_dataset.iterate(update=True):
                    result = self.f_gc(*inputs)
                    gradient += list_to_flat(
                        result[:len(self.p)]) / gradient_dataset.number_batches
                    costs.append(result[len(self.p):])

                print('cost=', np.mean(costs, axis=0), )
                print('lambda=%.5f,' % self.lambda_, )
                sys.stdout.flush()

                after_cost, flat_delta, backtracking, num_cg_iterations = self.cg(
                    -gradient)
                delta_cost = np.dot(flat_delta,
                                       gradient + 0.5 * self.batch_Gv(
                                           flat_delta,
                                           lambda_=0))  # disable damping
                ...
                if validation is not None and u % validation_frequency == 0:
                    if hasattr(validation, 'iterate'):
                        costs = np.mean(
                            [self.f_cost(*i) for i in validation.iterate()],
                            axis=0)
                    elif callable(validation):
                        costs = validation()
                    print('validation=', costs, )
                    if costs[0] < best[1]:
                        best = u, costs[0], [i.get_value().copy() for i in
                                             self.p]
                        print('*NEW BEST', )
                ...
                if isinstance(save_progress, str):
                    ...
                    with open(save_progress, 'wb') as fp:
                        pickle.dump(save, fp, -1)
...
class SequenceDataset:
    ...
    def __init__(self, data, batch_size, nbatches, minimum_size=10):
        '''SequenceDataset __init__

    data : list of lists of np arrays
    ...
    minimum_size : int
        ...
        self.number_batches = nbatches
        self.items = []

        for i_sequence in range(len(data[0])):
            if batch_size is None:
                self.items.append(
                    [data[i][i_sequence] for i in range(len(data))])
            else:
                for i_step in range(0,
                                    len(data[0][i_sequence]) - minimum_size + 1,
                                    batch_size):
                    self.items.append(
